...
try:
    ...
    OCR_AVAILABLE = False
    print("Warning: OCR libraries not available. Only text-based PDFs will be supported.")
        ...
        try:
            ...
            if len(extracted_text.strip()) > 50:  # Arbitrary threshold
                ...
                if not OCR_AVAILABLE:
                    print("❌ OCR libraries not available. Cannot process image-based PDFs.")
                    return ""
...
"""
    ...
    def _map_groq_response(self, raw_data: Dict) -> Dict:
        ...
        for groq_key, db_key in mapping.items():
            if groq_key in raw_data:
                ...
                if db_key in ['education_details', 'work_experience', 'skills', 'certifications', 
                              'projects', 'languages_spoken', 'hobbies_interests', 'achievements']:
                    if value is None:
                        mapped_data[db_key] = []
        ...
        if 'work_experience' in mapped_data and not isinstance(mapped_data['work_experience'], list):
            mapped_data['work_experience'] = [mapped_data['work_experience']] if mapped_data['work_experience'] else []
        
        # Ensure education_details is a list
        if 'education_details' in mapped_data and not isinstance(mapped_data['education_details'], list):
            mapped_data['education_details'] = [mapped_data['education_details']] if mapped_data['education_details'] else []
    ...
    def _generate_analysis(self, structured_data: Dict, resume_text: str) -> Dict:
        ...
        strengths, weaknesses = self._identify_strengths_weaknesses(structured_data, scores)
    ...
    def _generate_summary(self, data: Dict, scores: Dict) -> Dict:
        ...
        return {
            ...
            'total_skills': len(data.get('skills', [])) if isinstance(data.get('skills'), list) else 0,
            'total_experience': len(data.get('work_experience', [])) if isinstance(data.get('work_experience'), list) else 0,
            'total_projects': len(data.get('projects', [])) if isinstance(data.get('projects'), list) else 0,
            'total_education': len(data.get('education_details', [])) if isinstance(data.get('education_details'), list) else 0,
            'has_contact_info': bool(data.get('email_address') and data.get('phone_number')),
