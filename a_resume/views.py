...
sys.path.append(
    os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "Analyze")
)
...
def dashboard(request):
    ...
    completed_analyses = ResumeAnalysis.objects.filter(
    ...
    if completed_analyses.exists():
        ...
        total_suggestions = sum(
            len(analysis.recommendations) for analysis in completed_analyses
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.error(
                    request, f"Resume uploaded but analysis failed: {str(e)}"
                )
...
def analysis_results(request, analysis_id):
    ...
    if analysis.status in ["pending", "processing"]:
        return render(
            request, "a_resume/analysis_processing.html", {"analysis": analysis}
        )
    ...
    if analysis.work_experience:
        ...
        for exp in analysis.work_experience:
            if isinstance(exp, dict):
                ...
                formatted_exp["job_title"] = exp.get(
                ...
                formatted_exp["description"] = exp.get(
    ...
    if analysis.education_details:
        ...
        for edu in analysis.education_details:
            if isinstance(edu, dict):
                ...
                formatted_edu["institution"] = edu.get(
                        ...
                        if isinstance(skill_list, list):
                            for skill_item in skill_list:
                                if isinstance(skill_item, str):
                                    ...
                                    skill_name = skill_item.get(
                                        "name", skill_item.get("skill", str(skill_item))
                                    )
                                    if skill_name:
                            ...
                            skill_name = skill_list.get(
                                "name", skill_list.get("skill", str(skill_list))
                            )
                            if skill_name:
                                formatted_skills.append(skill_name)
                            ...
                            if isinstance(skill_list, list):
                                for skill_item in skill_list:
                                    if isinstance(skill_item, str):
                                        ...
                                        skill_name = skill_item.get(
                                            "name",
                                            skill_item.get("skill", str(skill_item)),
                                        )
                                        if skill_name:
                                            formatted_skills.append(skill_name)
                            elif isinstance(skill_list, str):
    ...
    if analysis.projects:
        ...
        for proj in analysis.projects:
            if isinstance(proj, dict):
                ...
                formatted_proj["description"] = proj.get(
                    "Description", proj.get("description", "")
                )
                formatted_proj["duration"] = proj.get(
                    "Duration", proj.get("duration", "")
                )
                formatted_proj["technologies"] = proj.get(
    ...
    if request.method == "POST":
        ...
        if analysis.file:
            try:
                ...
                print(
                    f"Successfully deleted file from Cloudinary: {analysis.file.name}"
                )
...
def perform_ai_analysis(resume_analysis):
    ...
    try:
        ...
        try:
            ...
            raise Exception(
                f"Could not read file from Cloudinary storage: {storage_error}"
            )
        ...
        try:
            ...
            resume_analysis.education_details = (
                extracted_data.get("education_details") or []
            )
            resume_analysis.work_experience = (
                extracted_data.get("work_experience") or []
            )
            ...
            resume_analysis.languages_spoken = (
                extracted_data.get("languages_spoken") or []
            )
            resume_analysis.hobbies_interests = (
                extracted_data.get("hobbies_interests") or []
            )
