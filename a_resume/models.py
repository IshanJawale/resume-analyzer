...
class UserProfile(models.Model):
    ...
    def get_average_score(self):
        ...
        analyses = self.user.resume_analyses.filter(
            status="completed", overall_score__isnull=False
        )
        if analyses:
            return round(
                analyses.aggregate(models.Avg("overall_score"))["overall_score__avg"]
                or 0
            )
        return 0
    ...
    def get_best_score(self):
        ...
        if analyses:
            return (
                analyses.aggregate(models.Max("overall_score"))["overall_score__max"]
                or 0
            )
        return 0
...
class AnalysisRecommendation(models.Model):
    ...
    category = models.CharField(
        max_length=100
    )  # e.g., 'content', 'formatting', 'keywords'
